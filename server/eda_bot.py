...
from config_eda import BOT_TOKEN
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[
        InlineKeyboardButton(text="Global-Бот",
                             url="t.me/MainCo6akaBot")
    ]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    ans = f'''/start — начать работу с ботом\n
        /help — показать справку\n
        /find — найти магазины с едой рядом\n'''

    await update.message.reply_text(ans, reply_markup=reply_markup)
...
async def location_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for store in stores:
        ...
        if os.path.exists(ICON_PATH):
            await update.message.reply_photo(photo=InputFile(ICON_PATH),
                                             caption=text)
        else:
...
def main_eda():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ASK_LOCATION: [MessageHandler(filters.LOCATION, location_handler)]},
        fallbacks=[],
    )
